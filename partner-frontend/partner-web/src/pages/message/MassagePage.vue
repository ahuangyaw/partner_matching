<template>
    ...
    <van-cell @click="toHallChat">
        ...
        <template #label>
            <span class="cell-span">小黄友人官方聊天室</span>
        </template>
        <template #icon>
            <div class="icon_area">
    ...
    <van-cell-group v-for="team in teamList">
        <van-cell @click="toChatRoom(team.id,team.name)">
            ...
            <template #label>
                <span class="cell-span2" v-if="isNewMessage" >{{isNewMessage}}</span>
            </template>
            <template #icon>
                <van-image :src="team.coverImage || defaultImg" round width="60" height="60" style="margin-left: 10px"/>
...
onMounted(async () => {
  ...
  let res4 = await myAxios.get("/message");
  if (res4?.data.code === 0) {
    isNewMessage.value = Number(res4.data.data)
  }
})
