<template>
    <van-search v-model="searchText" placeholder="搜索部落" @search="onSearch"/>
    ...
    <van-empty image="search" v-if="(!teamList || teamList.length===0) && !listLoading" description="暂无符合要求的部落"/>
...
const listTeams = async (currentPage, val = '', status = 0) => {
    ...
    if (res?.data.code === 0) {
        ...
        showFailToast("部落加载失败" + (res.data.description ? `,${res.data.description}` : ''))
    }
