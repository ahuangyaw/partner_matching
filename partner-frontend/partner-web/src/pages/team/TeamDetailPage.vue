<template>
    ...
    <van-cell-group inset>
        ...
        <van-cell title="部落名" icon="flag-o" :value="team.name"/>
        <van-cell title="部落描述" icon="label-o" :value="team.description"/>
        <van-cell title="队长" icon="manager-o" :value="team.leaderName"/>
        <van-cell title="部落状态" icon="shield-o" :value="teamStatusEnum[team.status]"/>
        <van-cell title="部落人数" icon="friends-o" :value="`${team.hasJoinNum}/${team.maxNum}`"/>
        <van-cell title="过期时间" icon="clock-o" :value="team.expireTime || '永不过期'"/>
        <van-cell title="部落聊天室" icon="chat-o" is-link @click="toChat" v-if="team.hasJoin"/>
    </van-cell-group>
    <van-divider
...
onMounted(async () => {
    ...
    if (res?.data.code === 0) {
        ...
        if (res_.data.code === 0) {
          ...
          showFailToast("获取部落成员失败" + (res.data.description ? `,${res.data.description}` : ''))
        }
    } else {
      showFailToast("部落查询失败" + (res.data.description ? `,${res.data.description}` : ''))
    }
    loading.value = false
})
