...
const joinTeam = async (teamId, password = "") => {
    ...
    if (res?.data.code === 0) {
        showSuccessToast("加入部落成功");
        onRefresh();
    } else {
        showFailToast("加入部落失败" + (res.data.description ? `,${res.data.description}` : ""));
    }
    doClear();
};
...
const doQuitTeam = async (id: number) => {
    ...
    if (res?.data.code === 0) {
        showSuccessToast("退出部落成功");
        onRefresh();
    } else {
        showFailToast("退出部落失败" + (res.data.description ? `,${res.data.description}` : ""));
    }
};
...
const doDeleteTeam = async (id: number) => {
    showConfirmDialog({
        ...
        .then(async () => {
            ...
            if (res?.data.code === 0) {
                showSuccessToast("解散部落成功");
                onRefresh();
            } else {
                showFailToast("解散部落失败" + (res.data.description ? `,${res.data.description}` : ""));
            }
        })
        .catch(() => {
        });
