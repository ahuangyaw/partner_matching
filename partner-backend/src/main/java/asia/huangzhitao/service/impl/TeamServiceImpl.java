...
import asia.huangzhitao.mapper.TeamMapper;
...
import asia.huangzhitao.model.enums.TeamStatusEnum;
import asia.huangzhitao.model.request.TeamCoverUpdateRequest;
import asia.huangzhitao.model.request.TeamJoinRequest;
import asia.huangzhitao.model.request.TeamQueryRequest;
import asia.huangzhitao.model.request.TeamQuitRequest;
import asia.huangzhitao.model.request.TeamUpdateRequest;
import asia.huangzhitao.model.vo.TeamVO;
...
import static asia.huangzhitao.constants.TeamConstants.MAXIMUM_DESCRIPTION_LEN;
import static asia.huangzhitao.constants.TeamConstants.MAXIMUM_JOINED_TEAM;
import static asia.huangzhitao.constants.TeamConstants.MAXIMUM_JOINED_USER_AVATAR_NUM;
import static asia.huangzhitao.constants.TeamConstants.MAXIMUM_MEMBER_NUM;
import static asia.huangzhitao.constants.TeamConstants.MAXIMUM_PASSWORD_LEN;
import static asia.huangzhitao.constants.TeamConstants.MAXIMUM_TEAM_NUM;
import static asia.huangzhitao.constants.TeamConstants.MAXIMUM_TITLE_LEN;

/**
 * 团队服务impl
 *
 * @author huang
 * @description 针对表【team(部落)】的数据库操作Service实现
...
public class TeamServiceImpl extends ServiceImpl<TeamMapper, Team> implements TeamService {
    ...
    public long addTeam(Team team, User loginUser) {
        ...
        if (!result || teamId == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "创建部落失败");
        }
        // 9. 插入用户  => 部落关系到关系表
    ...
    public void validateTeamParam(Long userId, Team team) {
        ...
        if (hasTeamNum >= MAXIMUM_TEAM_NUM) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户最多创建 5 个部落");
        }
        //   1. 部落人数 > 1 且 <= 20
        int maxNum = Optional.ofNullable(team.getMaxNum()).orElse(0);
        if (maxNum < 1 || maxNum > MAXIMUM_MEMBER_NUM) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "部落人数不满足要求");
        }
        //   2. 部落标题 <= 20
        String name = team.getName();
        if (StringUtils.isBlank(name) || name.length() > MAXIMUM_TITLE_LEN) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "部落标题不满足要求");
        }
        //   3. 描述 <= 512
        String description = team.getDescription();
        if (StringUtils.isNotBlank(description) && description.length() > MAXIMUM_DESCRIPTION_LEN) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "部落描述过长");
        }
        //   4. status 是否公开（int）不传默认为 0（公开）
        int status = Optional.ofNullable(team.getStatus()).orElse(0);
        TeamStatusEnum statusEnum = TeamStatusEnum.getEnumByValue(status);
        if (statusEnum == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "部落状态不满足要求");
        }
    ...
    public boolean updateTeam(TeamUpdateRequest teamUpdateRequest, User loginUser) {
        ...
        if (oldTeam == null) {
            throw new BusinessException(ErrorCode.NULL_ERROR, "部落不存在");
        }
        // 只有管理员或者部落的创建者可以修改
    ...
    public boolean joinTeam(TeamJoinRequest teamJoinRequest, User loginUser) {
        ...
        if (expireTime != null && expireTime.before(new Date())) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "部落已过期");
        }
        Integer status = team.getStatus();
        TeamStatusEnum teamStatusEnum = TeamStatusEnum.getEnumByValue(status);
        if (TeamStatusEnum.PRIVATE.equals(teamStatusEnum)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "禁止加入私有部落");
        }
        ...
        if (hasJoinNum > MAXIMUM_JOINED_TEAM) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "最多创建或加入 5 个部落");
        }
        // 不能重复加入已加入的部落
        ...
        if (hasUserJoinTeam > 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户已加入该部落");
        }
        // 已加入部落的人数
        long teamHasJoinNum = this.countTeamUserByTeamId(teamId);
        if (teamHasJoinNum >= team.getMaxNum()) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "部落已满");
        }
        // 修改部落信息
        UserTeam userTeam = new UserTeam();
        userTeam.setUserId(userId);
    ...
    public boolean quitTeam(TeamQuitRequest teamQuitRequest, User loginUser) {
        ...
        if (count == 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "未加入部落");
        }
        long teamHasJoinNum = this.countTeamUserByTeamId(teamId);
        // 部落只剩一人，解散
        if (teamHasJoinNum == 1) {
            ...
            if (team.getUserId() == userId) {
                ...
                if (!result) {
                    throw new BusinessException(ErrorCode.SYSTEM_ERROR, "更新部落队长失败");
                }
            }
        }
        // 移除关系
    ...
    public boolean deleteTeam(long id, User loginUser, boolean isAdmin) {
        ...
        if (isAdmin) {
            ...
            if (!result) {
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, "删除部落关联信息失败");
            }
    ...
    public TeamVO getTeam(Long teamId, Long userId) {
        ...
        if (team == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "部落不存在");
        }
